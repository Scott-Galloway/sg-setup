...
"${IPT}" -A INPUT -m set --match-set blacklist src -j LOG \
    --log-prefix "IP Blacklist: "
"${IPT}" -A INPUT -m set --match-set blacklist src -j DROP

# IPSET Output Blocklist - allows reject packet to be sent with no log
# but no further communication
"${IPT}" -A OUTPUT -m set --match-set blacklist dst -p icmp \
    --icmp-type port-unreachable -j ACCEPT
"${IPT}" -A OUTPUT -m set --match-set blacklist dst -j LOG \
    --log-prefix "IP Blacklist: "
"${IPT}" -A OUTPUT -m set --match-set blacklist dst -j REJECT \
...
"${IPT}" -A INPUT -i eth0 -p tcp --sport "${SSHPORT}"  -m conntrack \
    --ctstate ESTABLISHED -j ACCEPT
"${IPT}" -A OUTPUT -o eth0 -p tcp --dport "${SSHPORT}"  -m conntrack \
...
"${IPT}" -A INPUT -i eth0 -p tcp --sport 25 --dport 1024:65535 -m conntrack \
    --ctstate NEW,ESTABLISHED -j ACCEPT
"${IPT}" -A OUTPUT -o eth0 -p tcp --sport 1024:65535 --dport 25 -m conntrack \
    --ctstate NEW,ESTABLISHED -j ACCEPT

# Allow rsync from a specific network
#"${IPT}" -A INPUT -i eth0 -p tcp -s 192.168.101.0/24 --dport 873 -m conntrack \
#    --ctstate NEW,ESTABLISHED -j ACCEPT
#"${IPT}" -A OUTPUT -o eth0 -p tcp --sport 873 -m conntrack \
#    --ctstate ESTABLISHED -j ACCEPT
...
# They won't have to worry about DDoS rulesets
# "${IP6T}" -N ADMIN_IP
#"${IP6T}" -A ADMIN_IP -p tcp -m multiport --sports \
#    "${SSHPORT}",25,80,443,10050,10051 -j ACCEPT
#"${IP6T}" -A ADMIN_IP -p tcp -m multiport --dports \
#    "${SSHPORT}",25,80,443,10050,10051 -j ACCEPT
#"${IP6T}" -A ADMIN_IP -i eth0 -p icmp --icmp-type destination-unreachable \
#    -m limit --limit  1/s --limit-burst 1 -j ACCEPT
#"${IP6T}" -A ADMIN_IP -i eth0 -p icmp --icmp-type time-exceeded -m limit \
#    --limit  1/s --limit-burst 1 -j ACCEPT
#"${IP6T}" -A ADMIN_IP -i eth0 -p icmp --icmp-type echo-reply -m limit \
#    --limit  1/s --limit-burst 1 -j ACCEPT
#"${IP6T}" -A ADMIN_IP -i eth0 -p icmp --icmp-type echo-request -m limit \
#    --limit  1/s --limit-burst 1 -j ACCEPT
#"${IP6T}" -A ADMIN_IP -i eth0 -p icmp -m limit --limit 1/s --limit-burst 1 \
#    -j LOG --log-prefix "iptables: PING-DROP: "
...
# Allow certain inbound ICMP types (ping, traceroute..)
# "${IP6T}" -A INPUT -i eth0 -p icmp --icmp-type destination-unreachable \
#   -m limit --limit  1/s --limit-burst 1 -j ACCEPT
# "${IP6T}" -A INPUT -i eth0 -p icmp --icmp-type time-exceeded -m limit \
#   --limit  1/s --limit-burst 1 -j ACCEPT
# "${IP6T}" -A INPUT -i eth0 -p icmp --icmp-type echo-reply -m limit \
#   --limit  1/s --limit-burst 1 -j ACCEPT
# "${IP6T}" -A INPUT -i eth0 -p icmp --icmp-type echo-request -m limit \
#   --limit  1/s --limit-burst 1 -j ACCEPT
# "${IP6T}" -A INPUT -i eth0 -p icmp -m limit --limit 1/s --limit-burst 1 \
#   -j LOG --log-prefix "ip6tables: PING-DROP: "
...
# kill off identd quick 
# "${IP6T}" -A INPUT -i eth0 -p tcp --dport 113 -j REJECT \
#   --reject-with tcp-reset
...
# Multi-out for inbound SSH, HTTP, and HTTPS
#"${IP6T}" -A OUTPUT -o eth0 -p tcp -m multiport --sports "${SSHPORT}",80,443 \
#    -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT

# Outbound SSH
"${IP6T}" -A INPUT -i eth0 -p tcp --sport "${SSHPORT}"  -m conntrack \
    --ctstate ESTABLISHED -j ACCEPT
"${IP6T}" -A OUTPUT -o eth0 -p tcp --dport "${SSHPORT}"  -m conntrack \
    --ctstate NEW,ESTABLISHED -j ACCEPT
...
#    --ctstate NEW,ESTABLISHED -j ACCEPT
#"${IP6T}" -A OUPUT -o eth0 -p tcp --sport 25 --dport 1024:65535 -m conntrack \
#    --ctstate NEW,ESTABLISHED -j ACCEPT
...
#    -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT
#"${IP6T}" -A OUTPUT -o eth0 -p tcp --sport 873 -m conntrack \
#    --ctstate ESTABLISHED -j ACCEPT
